...
contract LetBetSportGameOverUnderBetting is Ownable {
    ...
    event BetWithOUDealer(address player, uint256 gameId, uint8 option, uint256 amount, uint256 betTimestamp, uint16[2] ouRate, uint16[2] winRateOver, uint16[2] winRateUnder);
    
    function LetBetSportGameOverUnderBetting(address _creditManager) public {
        creditManager = _creditManager;
    }
    
    function getInfo(uint256 _gameId) public constant returns (uint16[2] gameScore, uint8 result, uint8 status, uint256 betLimit, uint256 betTotalLimit, uint256 beginTimestamp, uint16[2][] ouRate, uint16[2][] winRateOver, uint16[2][] winRateUnder, uint numOfOUBetPlayers, uint256 ouBetTotal) {
        Game memory game = games[_gameId];
        return (game.gameScore, uint8(game.result), uint8(game.status), game.betLimit, game.betTotalLimit, game.beginTimestamp, game.ouRate, game.winRateOver, game.winRateUnder, game.ouBetPlayers.length, game.ouBetTotal);
    }
    
    function getPlayerOUBet(uint256 _gameId, uint _index) public constant returns (address addr, uint8 betOption, uint256 betAmount, uint256 winAmount, uint256 betTimestamp, uint16[2] ouRate, uint16[2] winRateOver, uint16[2] winRateUnder) {
        require(_index < games[_gameId].ouBetPlayers.length);
        
        OUBetPlayer memory player = games[_gameId].ouBetPlayers[_index];
        return (player.addr, uint8(player.betOption), player.betAmount, player.winAmount, player.betTimestamp, player.ouRate, player.winRateOver, player.winRateUnder);
    }
    ...
    function start(uint256 _gameId, uint256 _betLimit, uint256 _betTotalLimit, uint256 _beginTimestamp, uint16[2][] _ouRate, uint16[2][] _winRateOver, uint16[2][] _winRateUnder) onlyOwner public returns (bool success) {
    ...
    function performOUBet(uint256 _gameId, uint16[2] _gameScore) internal returns (bool success) {
        ...
        Game memory game = games[_gameId];
        for (i = 0; i < game.ouBetPlayers.length; ++i) {
            ouRateNatural = game.ouBetPlayers[i].ouRate[0] / MAGIC_NUMBER;
            ouRateDecimal = game.ouBetPlayers[i].ouRate[0] % MAGIC_NUMBER;
            ...
            amount = 0;
            betAmount = game.ouBetPlayers[i].betAmount;
            
            if (delta > 0 && game.ouBetPlayers[i].betOption == OUBetOption.OVER) { // over win -> bet over win
                amount = betAmount.mul(game.ouBetPlayers[i].winRateOver[0]).div(game.ouBetPlayers[i].winRateOver[1]);
            } else if (delta < 0 && game.ouBetPlayers[i].betOption == OUBetOption.UNDER) { // under win -> bet under win
                amount = betAmount.mul(game.ouBetPlayers[i].winRateUnder[0]).div(game.ouBetPlayers[i].winRateUnder[1]);
            } else if (delta == 0) { // draw
                if (ouRateDecimal == 0) { // over/under: 1, 2, 3,... -> stake refund
                    amount = betAmount;
                } else if (ouRateDecimal == 2 && game.ouBetPlayers[i].betOption == OUBetOption.UNDER) { // over/under: 0.5, 1.5, 2.5,... -> bet under win
                    amount = betAmount.mul(game.ouBetPlayers[i].winRateUnder[0]).div(game.ouBetPlayers[i].winRateUnder[1]);
                } else if (ouRateDecimal == 1) { // over/under: 1/1.5, 2/2.5, 3/3.5,... -> bet over half lose, bet under half win
                    if (game.ouBetPlayers[i].betOption == OUBetOption.OVER) {
                        amount = betAmount.div(2);
                    } else if (game.ouBetPlayers[i].betOption == OUBetOption.UNDER) {
                        amount = betAmount.add(betAmount.mul(game.ouBetPlayers[i].winRateUnder[0] - game.ouBetPlayers[i].winRateUnder[1]).div(game.ouBetPlayers[i].winRateUnder[1] * 2));
                    }
                } else if (ouRateDecimal == 3) { // over/under: 0.5/1, 1.5/2, 2.5/3,... -> bet over half win, bet under half lose
                    if (game.ouBetPlayers[i].betOption == OUBetOption.OVER) {
                        amount = betAmount.add(betAmount.mul(game.ouBetPlayers[i].winRateOver[0] - game.ouBetPlayers[i].winRateOver[1]).div(game.ouBetPlayers[i].winRateOver[1] * 2));
                    } else if (game.ouBetPlayers[i].betOption == OUBetOption.UNDER) {
                        amount = betAmount.div(2);
                    }
                }
            ...
            if (amount > 0) {
                if (!lbcm.increaseCredit(game.ouBetPlayers[i].addr, amount)) revert();
                game.ouBetPlayers[i].winAmount = amount;
            }
        }
    ...
    function betWithOUDealer(uint256 _gameId, uint8 _betOption, uint256 _betAmount, uint _ouRateIndex) public returns (bool success) {
        ...
        games[_gameId].ouBetPlayers.push(OUBetPlayer(msg.sender, OUBetOption(_betOption), _betAmount, 0, now, games[_gameId].ouRate[_ouRateIndex], games[_gameId].winRateOver[_ouRateIndex], games[_gameId].winRateUnder[_ouRateIndex]));
        games[_gameId].ouBetTotal = games[_gameId].ouBetTotal.add(_betAmount);
        
        BetWithOUDealer(msg.sender, _gameId, _betOption, _betAmount, now, games[_gameId].ouRate[_ouRateIndex], games[_gameId].winRateOver[_ouRateIndex], games[_gameId].winRateUnder[_ouRateIndex]);
